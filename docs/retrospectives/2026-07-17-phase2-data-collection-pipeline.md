...
- **`core/market_calendar.py` 신설.** "지금이 장운영 시간인가"는 API 계층의
  ...
  패널이 이를 `core/`로 승격시켜 단일 출처화했다(docs/reference/project-context.md §3의 계층 원칙:
...
Task 4 구현자가 docs/reference/project-context.md 규칙 8의 4-에이전트 패널을 **스스로 디스패치해
...
전부 docs/reference/project-context.md §5·spec §5에도 반영됨:
