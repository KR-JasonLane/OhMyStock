...
> R7(엔진 통합 예행)은 2026-07-23 완료 — §5 절 참조. R1 잔여(저유동
> 대체 심볼·브리지 프로브·페이지 경계)도 같은 날 실측 완료(커밋 03e8603).
...
## 5. R7 — 엔진 통합 예행 (2026-07-23 완료, 커밋 후 추가)

### 무엇을 했나
리플레이 프로필로 실제 백엔드를 기동해 Task 8 시나리오 전체를 speed=1.0
으로 예행했다(증거 `.superpowers/sdd/replay-r7-run1.txt`). 시드는
`scripts/replay_r7_seed.py` — 분봉 실측을 일봉으로 집계해 별도 DB에 주입
(가짜 값 없음, 검증용 analysis만 model='MANUAL-R7'로 명시 주입).

### 실증된 것 (run 1~4)
- **R6 기동 게이트 3종 라이브 발동**: override WARNING, /_replay/status
  프로브(speed 검증), 서버 소싱 앵커 — 오프셋 시계가 감사 타임스탬프까지
  관통(trade_runs.started_at이 재생 시각).
- **진입 전 경로**(run 1·2·4): 신선도→갭 가드→선정→지정가 발주→체결→
  잔고 대사 평단 확정. run_environment='replay' 분리 기록.
- **§9 결함 방어**(run 1): 429→어댑터 지수 백오프(1s→2s), 500×3→모니터
  연속 실패 카운팅, 전파 지연 5s 하 C1 경유 청산.
- **킬스위치**(run 1): LIQUIDATE_ALL 시장가 전량 청산+감사 4컬럼.
- **손절 자연 발동**(run 2): 000660 진입 2,396,000 → 재생 09:08 크로스 →
  시장가 청산 2,259,000, exit_reason='stop_loss' — **클라이언트 사이드
  손절(시스템 존재 이유)의 실측 데이터 end-to-end 첫 증명.**
- **크래시 재기동 reconcile**(run 4): 보유 중 프로세스 강제 종료 → 재기동
  → 주문 0건으로 포지션 승계 + "already held" 사유 스킵(당일 selection
  패치 실전 발동) → 복구 포지션이 자연 손절로 종결. 크래시 관통 전체
  수명주기 증명.

### R7이 찾아낸 결함/개선 (예행의 존재 이유)
1. **게이트 정의 불일치**: 커버리지 게이트의 "첫 손절 크로스"가 진행 고점
   대비 DD 기준 — 엔진은 진입가 대비. run 1 심볼 선정이 빗나감(035760
   당일 최저 -4.45%로 미발동). → 게이트를 "09:05 가정 진입가 대비"로
   재산정하도록 수정 필요(후속 태스크).
2. **리플레이 시계 혼합**: store 타임스탬프(실시계)와 서비스 시계(재생)가
   쿨다운 등 시간 비교에서 혼합됨 — run 3에서 재진입 쿨다운 오발동으로
   실측. → main.py가 리플레이 프로필 시 TradingStore에도 재생 시계 주입
   (후속 태스크).
3. (Task 8 라이브에서 발견되어 당일 수정 완료: selection 침묵 드랍 —
   커밋 ceb16c7. R7 run 4가 수정판의 실전 발동을 재확인.)

### Task 8(실서버)와의 관계
아침 실서버 런은 갭 가드(mock 일봉↔실시간 괴리 +5%)에 막혀 진입 없이
종료(그 자체로 가드 실증 — `p5-task-8-live.txt`). 진입~청산 실경로는 본
R7이 대체 검증했고, 실서버 "정상 통과" 재시도는 게이트 정의 수정 후
갭이 맞는 날 선택적으로 수행한다.
