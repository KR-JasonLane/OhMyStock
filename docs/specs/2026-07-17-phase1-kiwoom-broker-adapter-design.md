...
- **날짜:** 2026-07-17
- **상태:** 확정 — 구현 완료 (2026-07-17, §5 표는 실측 결과 반영)
...
```python
class Quote:      # 현재가: symbol, name, price, change_rate, volume
...
## 5. 검증된 팩트 vs 실측 필요 항목 (리서치 2026-07-17 → 실측 2026-07-17)

| 항목 | 상태 |
|---|---|
| 토큰: `POST /oauth2/token`, `grant_type=client_credentials`, 응답 `token`/`expires_dt`(절대시각)/`return_code` | **검증됨(실측).** `expires_dt`는 `YYYYMMDDHHMMSS` 형식의 KST 절대시각(상대 TTL 아님) — Task 3 구현이 그대로 파싱해 사용 |
| 토큰 폐기: `POST /oauth2/revoke` | **검증됨(실측).** `{appkey, secretkey, token}` 바디, 응답 `return_code`(0=성공) |
| TR 호출: 카테고리별 POST 엔드포인트(`/api/dostk/...`) + `api-id` 헤더, 연속조회 `cont-yn`/`next-key` 헤더 | **검증됨(실측).** 관측된 카테고리: `stkinfo`(현재가), `chart`(봉), `acnt`(계좌) |
| TR id: 현재가 `ka10001`, 일봉 `ka10081`, 예수금 `kt00001`, 계좌평가잔고 `kt00018` | **검증됨(실측).** 필드명까지 리서치와 정확히 일치(§4 아래 상세) |
| **레이트리밋 1 req/s per-TR** | **여전히 보류.** 공식 수치 미확인 — 레이트리미터를 설정값(rate/burst)으로 구현해 실측 없이도 후속 조정 가능하게 함 |
| **429 응답 바디 스키마** | 보류 — Task 6 라이브 스모크 연속 실행 중 토큰 발급 경로(`auth.py`)에서 429가 간헐 관측됐으나(TR 호출이 아니라 토큰 발급 몰림), 429 바디 스키마 자체를 구조적으로 캡처하지 않음. TR 호출 쪽(`client.py`)은 429 수신 시 `RateLimiter.penalize()` + 지수 백오프로 대응(코드는 존재, 바디 파싱에 의존하지 않음) |
| 모의서버 제약(일부 종목 주문 제한, 지정가·시장가만 등) | 주문은 Phase 5 — 이번엔 영향 없음(미실측) |
| 종목코드 거래소 접두(`KRX:005930`), NXT/SOR 구분 | **실측 정정.** 현재가/일봉 조회에서 접두 없는 순수 6자리 코드(`"005930"`)를 그대로 사용 — `KRX:` 접두는 불필요했음. 계좌 잔고(`kt00018`) 응답의 `stk_cd`에는 `"A"` 1글자 접두가 붙어 나와(`"A005930"`) 파싱 시 제거함. NXT/SOR 구분은 미실측(Phase 5+) |
| **(신규) `ka10081` `base_dt` 파라미터** | **실측 정정.** 리서치 당시 알려지지 않았던 필수 파라미터 — 빈 문자열 전달 시 `[1511:필수 입력 값이 존재하지 않습니다. 필수입력파라미터=base_dt]` 거부. 당일(KST) `YYYYMMDD`를 채워야 함. 일봉 원본 응답은 최신→과거 내림차순(실측: `['20260716','20260715','20260714']`) — 어댑터가 과거→최신으로 재정렬 |
| **(신규) 모의/실전 키 발급 분리** | **실측 정정.** 실전용 키로 모의서버 호출 시 HTTP 200 + `return_code=2` + `[8030:투자구분(실전/모의)이 달라서 Appkey를 사용할수가 없습니다]` — 모의 전용 키를 별도 발급해야 함(§Task 3, 회고록 참고) |
| **(신규) 계좌 조회 최상위 필드 존재 여부** | **검증됨(실측).** `kt00001`의 `entr`/`ord_alow_amt`, `kt00018`의 `tot_evlt_amt`/`tot_evlt_pl`는 포지션 0개 상태에서도 필드 자체는 존재(값 0) — hard indexing 가능 |
| **(신규) `kt00018` 행 단위 필드 / `avg_price` 반올림** | **보류.** 모의 계좌 포지션이 0개라 `acnt_evlt_remn_indv_tot` 배열 행 자체가 관측되지 않음 — **Phase 5 착수 전 PRE-GATE**(실제 포지션 필요, 검증용 라이브 테스트 `test_live_잔고_원본응답_avg_price_실측` 이미 존재) |
