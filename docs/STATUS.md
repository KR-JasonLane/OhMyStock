...
**Phase 5(트레이딩 엔진) 스펙 확정(2026-07-22, v3.1) — 다음은 구현
계획서 작성(`docs/plans/`).** 스펙
`docs/specs/2026-07-21-phase5-trading-engine-design.md`. 4-에이전트 패널을
**3라운드**(초안→v2→v3 델타) 돌려 v1의 Critical 5건(재기동 reconcile 부재,
BackgroundRunService 정지 계약 부재, 주문 원문 토큰 저장, 청산 체결 방식
미정의, 사이징 분모 모호)을 전부 소진, 신규 Critical 0으로 수렴. 결정 #25~#35
확정(진입=지정가+시장가 폴백, 균등분할 고정슬롯, REST 폴링 감시, 시가안정화
진입, 트레일링 활성화 후 고정익절 해제+선형보간 폭, 5종목/50% 한도, 오버나이트
허용, max_holding_days=10, 주문전용 스코프토큰 P5로 당김). **착수 전 PRE-GATE
G1~G4(장중 실행 필요):** G1 ka10095 다종목시세+호가, G2 주문TR 3종+레이트리밋
버킷, G3 kt00018 행단위 필드(기존 #1), G4 모의키→실전엔드포인트 대칭성.
**본편 선행 태스크:** BackgroundRunService 정지계약+승격 확장(트레이딩 구현보다
먼저). 테스트 현황 **319 passed, 10 deselected**(스펙 단계라 미변동).
...
[ ] Phase 5: 트레이딩 엔진 구현 계획서 작성                    <-- 다음
    (선행 태스크: BackgroundRunService 정지계약+승격 확장. PRE-GATE 장중 실측)
