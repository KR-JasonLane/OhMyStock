...
**Phase 4(AI 멀티에이전트 분석) 완료 — Phase 5(트레이딩 엔진) 착수 전
PRE-GATE 처리부터 재개.** Phase 5 PRE-GATE 목록은 아래 "Phase 5 착수 전"
블록 참고 — 특히 ① `max_picks_advice` 미저장(감사 불가), ② economist
폴백 열림/닫힘 **사용자 결정 1건 대기**(P4 회고록 §7), ③ kt00018
실포지션 실측(hard gate)이 선행이다.
...
- ⚠️ **PRE-GATE — Phase 5(트레이딩 엔진) 착수 전 반드시 먼저 처리(hard gate):**
  1. `kt00018` 행 단위 필드 실측 — 모의 계좌에 실제 포지션을 만든 뒤
     `stk_cd`/`pur_pric`/`cur_prc` 등과 `avg_price` 원 단위 반올림 가정을
     검증. 라이브 테스트(`test_live_잔고_원본응답_avg_price_실측`) 기존재.
  2. **`max_picks_advice` DB 저장**(analysis_runs 칼럼 추가 + 마이그레이션)
     — "approve인데 픽 0"을 감사할 수 없는 상태로 실주문 연동 금지
     (P4-T7 트레이더 패널).
  3. **economist 파싱 실패 폴백 열림/닫힘 사용자 결정** — 현행 스펙은
     neutral+advice=max_picks(열림), 트레이더 패널은 advice=0(닫힘) 권고.
     P4 회고록 §7.
  4. `/analyze/status` 타임스탬프 + `/analyze/latest`에
     `score_reference_date` 추가 — 소비자(Phase 5/7)가 낡은 결과를 오늘
     것으로 오인하는 경로 차단 (P4-T6 트레이더 패널).
  5. 프롬프트 거래비용 상수(0.2~0.3%p)를 실제 비용 설정과 연동 + 백테스트
     한계(겹침 표본·regime 미조건화) 프롬프트 명시 (P4-T7 트레이더 패널).
  6. 쓰기 엔드포인트(`POST /collect`·`/score`·`/analyze`) 인증/CORS 오리진
     제한 도입 여부 확정 (P3 T7·P4 T6 보안 패널).
...
[ ] Phase 5: 트레이딩 엔진                                    <-- 다음 (착수 전:
    PRE-GATE 7건 — 위 "Phase 5 착수 전" 블록. 사용자 결정 1건 포함)
