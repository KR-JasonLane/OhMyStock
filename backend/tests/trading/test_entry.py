...
from app.domain.broker import (MarketData, OpenOrder, OrderAck, OrderSide,
                               OrderStyle, Quote)
...
class FakeOrders:
    ...
    open_orders_script: get_open_orders 호출별 시나리오 — 정수=미체결 잔량,
    None=주문 부재 관측, Exception=조회 실패. **소진 시 AssertionError**
    (시나리오 길이 오산이 '전량 체결'로 수렴해 허위 통과하는 것 방지 —
    개발자 Minor. 폴 루프가 예외를 삼키므로 테스트는 결과 단언에서 깨진다).
    cancel_script: cancel_order 호출별 예외(None=성공). 소진 후 호출은 성공.
    requote_ask: get_quotes 응답 ask. None=조회 실패(stale 폴백 경로)."""
    ...
    async def get_open_orders(self):
        ...
        return [OpenOrder(order_no=f"ORD{self._order_seq}", symbol="005930",
                          side=OrderSide.BUY, order_qty=PLAN.quantity,
...
async def test_시장가_재발주는_시세를_재조회해_caps와_평단에_반영():
    ...
    fake = FakeOrders(open_orders_script=[10, 10, 10, None, None],
                      requote_ask=280_000)
    out = await _executor(fake, caps=calls.append).execute(PLAN, ask=273_500)
...
async def test_caps는_발주_직전_호출되고_거부시_주문_안나감():
    ...
    assert calls == [273_500 * 10]  # 지정가×수량 — 발주 직전 검증(§8-1)
