...
class TradingStore:
    ...
    def daily_order_usage(self, day: date,
        ...
        coarse_lo = (start_kst - timedelta(days=1)).astimezone(timezone.utc)
        coarse_hi = (start_kst + timedelta(days=2)).astimezone(timezone.utc)
        count = 0
        krw = 0
        has_buy = False
        with self._sessions() as session:
            ...
            for row in rows:
                ...
                if created.tzinfo is None:
                    # 프로덕션(Postgres, timezone=True)은 aware UTC — naive는
                    # sqlite 테스트 경로뿐이며 UTC 벽시계로 간주한다.
                    created = created.replace(tzinfo=timezone.utc)
                if created.astimezone(KST).date() != day:
                    continue
